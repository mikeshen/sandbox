...
public:
  ...
  // Default constructor
  Order()
      : orderId(0), isBuy(false), price(0.0), quantity(0), fillOrKill(false),
        timestamp(std::chrono::steady_clock::now()) {}

  // Parameterized constructor
  Order(int id, bool buy, double p, int q, bool fok)
      : orderId(id), isBuy(buy), price(p), quantity(q), fillOrKill(fok),
        timestamp(std::chrono::steady_clock::now()) {}

  // Copy constructor
  Order(const Order &other)
      : orderId(other.orderId), isBuy(other.isBuy), price(other.price),
        quantity(other.quantity), fillOrKill(other.fillOrKill),
...
class OrderBook {
  struct OrderNode {
    ...
    OrderNode()
        : order(0, false, 0.0, 0, false), orderList(nullptr) {
    } // Default constructor
  };
  ...
  void matchOrdersHelper(Order &incomingOrder, Iterator begin, Iterator end,
                         Comparator comp) {
    for (auto it = begin; it != end; ++it) {
      ...
      for (auto listIt = orderList.begin();
        ...
        int tradeQuantity =
            std::min(incomingOrder.quantity, existingOrder.quantity);
  ...
  void matchOrder(Order &incomingOrder) {
    ...
    if (incomingOrder.isBuy) {
      ...
      matchOrdersHelper(incomingOrder, matchingOrders.begin(),
                        matchingOrders.end(), std::less_equal<double>());
      ...
      matchOrdersHelper(incomingOrder, matchingOrders.rbegin(),
                        matchingOrders.rend(), std::greater_equal<double>());
    ...
    if (incomingOrder.fillOrKill && incomingOrder.quantity > 0) {
      ...
      std::cout << "Fill-or-kill order ID " << incomingOrder.orderId
                << " was cancelled.\n";
    }
