...
public:
    ...
    // Default constructor
    Order()
        : orderId(0), isBuy(false), price(0.0), quantity(0), fillOrKill(false), timestamp(std::chrono::steady_clock::now()) {}

    // Parameterized constructor
    Order(int id, bool buy, double p, int q, bool fok)
        : orderId(id), isBuy(buy), price(p), quantity(q), fillOrKill(fok), timestamp(std::chrono::steady_clock::now()) {}

    // Copy constructor
    Order(const Order& other)
        : orderId(other.orderId), isBuy(other.isBuy), price(other.price), quantity(other.quantity), fillOrKill(other.fillOrKill), timestamp(other.timestamp) {}

    // Move constructor
    Order(Order&& other) noexcept
        : orderId(other.orderId), isBuy(other.isBuy), price(other.price), quantity(other.quantity), fillOrKill(other.fillOrKill), timestamp(std::move(other.timestamp)) {
        other.orderId = 0;
        other.isBuy = false;
        other.price = 0.0;
        other.quantity = 0;
...
class OrderBook {
    ...
    void matchOrdersHelper(Order& incomingOrder, Iterator begin, Iterator end, Comparator comp) {
        for (auto it = begin; it != end; ++it) {
            ...
            for (auto listIt = orderList.begin(); listIt != orderList.end() && incomingOrder.quantity > 0;) {
                ...
                if (!comp(incomingOrder.price, existingOrder.price)) break;
    ...
    void matchOrder(Order& incomingOrder) {
        ...
        if (incomingOrder.isBuy) {
            ...
            matchOrdersHelper(incomingOrder, matchingOrders.begin(), matchingOrders.end(), std::less_equal<double>());
            ...
            matchOrdersHelper(incomingOrder, matchingOrders.rbegin(), matchingOrders.rend(), std::greater_equal<double>());
        ...
        if (incomingOrder.quantity > 0) {
            ...
            auto it = sameSideOrders[incomingOrder.price].insert(sameSideOrders[incomingOrder.price].end(), newNode);
            newNode.it = it;
